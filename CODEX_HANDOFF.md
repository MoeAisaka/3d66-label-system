...
1. 新增“抽样策略配置”：常规抽样比例、低置信度阈值、冷启动必审数量、高等级必审范围。
2. 队列默认支持按“当前模型×提示词组合 / 当前任务批次 / 时间范围”查看，避免所有历史结果一起进入今日待办。
3. 新增审核任务认领、审核人分配、占用状态和乐观锁，避免多人同时修改同一结果。
4. 增加批次进度：应审、已审、剩余、发现问题数、问题率和预计人力。
5. 人工审核完成后要自动从待办队列移出，并记录当时的抽样规则版本。
