...
export function SampleSetsPage() {
  ...
  const setStatus = useMutation({
    ...
    onSuccess: async () => { await refresh(); toast.success("黄金标准已锁定，将参与每次发布回归") },
    onError: (error) => toast.error(error.message),
  })
  ...
  const lockedReadOnly = detail.data?.summary.kind === "golden" && detail.data.summary.status === "locked"
  useEffect(() => {
    if (lockedReadOnly) setMode("included")
  }, [lockedReadOnly])
        ...
        <div className="grid gap-8 xl:grid-cols-[300px_minmax(0,1fr)]">
          ...
          <main className="min-w-0">
            {detail.data ? <>
              ...
              {historyItemId && <SampleInspector data={history.data} loading={history.isLoading} readOnly={lockedReadOnly} sampleSetId={selectedId!} onSaved={refresh} />}
...
function SampleRow({ item, selected, readOnly, onHistory, onRemove }: { item: SampleSetItem; selected: boolean; readOnly: boolean; onHistory: () => void; onRemove: () => void }) {
  ...
  return <article className={`grid gap-4 border-b border-[var(--line)] p-4 last:border-0 lg:grid-cols-[72px_minmax(200px,1fr)_110px_150px_140px] lg:items-center ${selected ? "bg-[#f7f9ef]" : ""}`}>
    ...
    <div className="flex flex-wrap justify-end gap-1"><Button size="sm" variant="secondary" onClick={onHistory}>{readOnly ? "查看标准与历史" : "标准与历史"}<ArrowRight /></Button>{!readOnly && <Button size="sm" variant="ghost" onClick={onRemove} aria-label="移出样本集"><Trash /></Button>}</div>
  </article>
}
...
function SampleInspector({ data, loading, readOnly, sampleSetId, onSaved }: { data?: SampleItemHistory; loading: boolean; readOnly: boolean; sampleSetId: number; onSaved: () => Promise<void> }) {
  const queryClient = useQueryClient()
  ...
  return <section className="mt-7 border-y border-[var(--line-strong)] bg-white">
    ...
    {tab === "truth" && <div className="p-5"><div className="grid gap-5 xl:grid-cols-[220px_1fr]">
      <div className="space-y-4"><label><FieldLabel>最终等级</FieldLabel><select disabled={readOnly} className="h-11 w-full rounded-[4px] border border-[var(--line-strong)] bg-white px-3 disabled:bg-[#f4f5f1]" value={truth.level || ""} onChange={(event) => setTruth({ ...truth, level: event.target.value })}><option value="">未定级</option>{["L1", "L2", "L3", "L4", "L5"].map((level) => <option key={level}>{level}</option>)}</select></label><label><FieldLabel>主分类</FieldLabel><Input disabled={readOnly} value={truth.category || ""} onChange={(event) => setTruth({ ...truth, category: event.target.value })} /></label><label><FieldLabel>画质问题</FieldLabel><select disabled={readOnly} className="h-11 w-full rounded-[4px] border border-[var(--line-strong)] bg-white px-3 disabled:bg-[#f4f5f1]" value={truth.quality_severity || "uncertain"} onChange={(event) => setTruth({ ...truth, quality_severity: event.target.value })}>{[["normal", "画质正常"], ["slight", "轻微"], ["moderate", "中度"], ["severe", "严重"], ["unusable", "不可用"], ["uncertain", "不确定"]].map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
      <div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold">{truthSchemaBound ? `${truthDefinitions.length} 个美感维度` : "历史维度记录"}</p>{truthSchemaBound ? <Badge tone="active">{truth.dimension_schema?.schema_key} · {truth.dimension_schema?.version}</Badge> : <Badge tone="warning">规则未绑定，仅供查看</Badge>}</div>{truthSchemaBound ? <div className="mt-2 grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2">{truthDefinitions.map(({ key, label }) => <label key={key} className="flex items-center justify-between gap-3 bg-white px-3 py-3"><span className="text-xs">{label}</span><select disabled={readOnly} className="h-9 w-16 rounded-[4px] border border-[var(--line)] bg-white px-2 text-sm font-semibold disabled:bg-[#f4f5f1]" value={truth.dimensions?.[key] || ""} onChange={(event) => updateDimension(key, Number(event.target.value))}><option value="">—</option>{[1, 2, 3, 4, 5].map((grade) => <option key={grade}>{grade}</option>)}</select></label>)}</div> : <div className="mt-2 grid gap-px border border-[var(--line)] bg-[var(--line)] sm:grid-cols-2">{Object.entries(truth.dimensions || {}).map(([key, grade]) => <div key={key} className="flex items-center justify-between gap-3 bg-white px-3 py-3"><span className="font-data text-xs">{key}</span><span className="text-sm font-semibold">{grade || "—"}</span></div>)}</div>}<p className="mt-5 text-xs font-semibold">图片形态</p><div className="mt-2 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{MEDIA_FIELDS.map(([key, label]) => <label key={key}><span className="mb-1 block text-xs text-[var(--muted)]">{label}</span><select disabled={readOnly} className="h-9 w-full rounded-[4px] border border-[var(--line)] bg-white px-2 text-xs disabled:bg-[#f4f5f1]" value={truth.media_form?.[key] || "uncertain"} onChange={(event) => updateMedia(key, event.target.value as "yes" | "no" | "uncertain")}><option value="yes">是</option><option value="no">否</option><option value="uncertain">不确定</option></select></label>)}</div></div>
    </div>{readOnly ? <div className="mt-6 border-t border-[var(--line)] pt-5 text-xs text-[var(--muted)]">当前为锁定只读版本，可查看完整真值、来源和修订历史；调整请在后继草稿中进行。</div> : <div className="mt-6 grid gap-4 border-t border-[var(--line)] pt-5 lg:grid-cols-[1fr_1fr_auto] lg:items-end"><label><FieldLabel>标准备注</FieldLabel><Input value={note} onChange={(event) => setNote(event.target.value)} placeholder="记录难点和判断依据" /></label><label><FieldLabel>本次修改原因</FieldLabel><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="例如：人工复核确认画质应为中度异常" /></label><Button onClick={() => save.mutate()} disabled={save.isPending}>保存为 V{data.item.truth_revision + 1}</Button></div>}</div>}
