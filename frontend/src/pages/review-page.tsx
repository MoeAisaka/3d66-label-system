                ...
                {evaluation.preprocess?.category_key === "pdf_text" && (
                  <div className="border-b border-[var(--line)] bg-[#f5f8fb] px-5 py-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <Badge tone={["unavailable", "partial"].includes(evaluation.preprocess.pdf?.ocr_status ?? "") ? "warning" : "success"}>OCR {evaluation.preprocess.pdf?.ocr_status === "completed" ? "已执行" : evaluation.preprocess.pdf?.ocr_status === "partial" ? "部分完成" : evaluation.preprocess.pdf?.ocr_status === "unavailable" ? "不可用" : "无需执行"}</Badge>
                    </div>
                    {evaluation.preprocess.multimodal_summary && <div className="mt-3 border-y border-[var(--line)] bg-white px-4 py-3"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-bold">多模态总结 · {evaluation.preprocess.multimodal_summary.document_type || "未分类文档"}</p><span className="font-data text-[0.68rem] text-[var(--muted)]">置信度 {Math.round(evaluation.preprocess.multimodal_summary.confidence * 100)}% · {evaluation.preprocess.multimodal_summary.model_id}</span></div><p className="mt-2 text-xs leading-5 text-[var(--muted)]">{evaluation.preprocess.multimodal_summary.summary}</p></div>}
                    {evaluation.preprocess.text_excerpt && <details className="mt-3"><summary className="cursor-pointer text-xs font-semibold">查看文本摘录</summary><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap border border-[var(--line)] bg-white p-3 text-xs leading-5 text-[var(--muted)]">{evaluation.preprocess.text_excerpt}</pre></details>}
                  </div>
                )}
                {sampling && (
