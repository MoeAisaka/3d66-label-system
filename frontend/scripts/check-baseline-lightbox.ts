import assert from "node:assert/strict"
import { existsSync, mkdtempSync, rmSync } from "node:fs"
import { tmpdir } from "node:os"
import path from "node:path"
import { spawn, spawnSync } from "node:child_process"
import { fileURLToPath } from "node:url"

const frontendRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..")
const viteEntry = path.join(frontendRoot, "node_modules", "vite", "bin", "vite.js")
const port = 4174
const testUrl = `http://127.0.0.1:${port}/scripts/lightbox-test.html`
const chromeCandidates = [
  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome",
  "/Applications/Chromium.app/Contents/MacOS/Chromium",
  "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe",
  "C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe",
]
const chrome = chromeCandidates.find(existsSync)

assert(chrome, "未找到可用于 lightbox 交互测试的 Chrome/Chromium")

const vite = spawn(
  process.execPath,
  [viteEntry, "--host", "127.0.0.1", "--port", String(port), "--strictPort"],
  { cwd: frontendRoot, stdio: "ignore" },
)
const profileDir = mkdtempSync(path.join(tmpdir(), "3d66-lightbox-test-"))

async function waitForVite() {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    try {
      const response = await fetch(testUrl)
      if (response.ok) return
    } catch {
      // Vite is still starting.
    }
    await new Promise((resolve) => setTimeout(resolve, 100))
  }
  throw new Error("lightbox 测试页未能启动")
}

try {
  await waitForVite()
  const result = spawnSync(
    chrome,
    [
      "--headless=new",
      "--disable-gpu",
      "--no-sandbox",
      `--user-data-dir=${profileDir}`,
      "--virtual-time-budget=3000",
      "--dump-dom",
      testUrl,
    ],
    { cwd: frontendRoot, encoding: "utf8", timeout: 15000 },
  )
  assert.equal(result.status, 0, "Chrome lightbox 测试进程失败")
  assert.match(
    result.stdout,
    /data-test-status="passed"/,
    "lightbox 浏览器交互契约未通过",
  )
  console.log("baseline lightbox browser contract: ok (open, src, Escape close)")
} finally {
  vite.kill("SIGTERM")
  rmSync(profileDir, { recursive: true, force: true })
}
