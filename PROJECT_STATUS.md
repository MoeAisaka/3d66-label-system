...
> 本轮开发基线：`7331399`。已推送至 Codeup：`main` 最新代码提交 `0d6d69e`、
> `windows-deploy` 为对应同步合并提交（均为快进，未使用 force push）。已在
> MacBook 以隔离 `DATA_DIR` 的 8099 金丝雀完成真实浏览器验收，1280/1440/
> 1536/1920/390px 五个视口文档级横向溢出均为 0；生产 8080 仍为 `5c25bfa`，
> 是否提升待操作员决定。公司 Windows 实例由公司前端工程师后续按
> `windows-deploy` 分支自行部署。
