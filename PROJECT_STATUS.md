...
## 最新实施：统一美感基础分与机制候选发布门禁（2026-08-21，待发布）

- 所有图片类目统一以调用 B 的合法 `aesthetic_score` 作为等级撮合器初始分，再执行合同命中扣分、媒介修正、赛道封顶和等级映射；缺少合法分数或证据时失败关闭，返回 `score=null`、`level=null`、`needs_review=true`，并兼容 `aesthetic_evidence`、`overall_evidence` 与维度证据。
- 调用 A 的红线信号同时具备规则值、布尔信号和非空证据时直接前置过滤为 L5，不调用 B；证据不完整时继续正常进入 B。候选回归完成后增加统一质量门禁、CAS/祖先链/快照/提示词绑定/基准集/字段指标校验和管理员显式启用接口。
- 3D/SU 新增 v4 模型、提示词、基础美感合同与等级规则 seed，旧版本保留且不覆盖历史 revision；纠偏候选回归页支持 URL 选中候选并在门禁通过后启用。
- 本地验证：后端全量 `1681 passed, 1 skipped, 6 warnings`；前端 `npm run build` 通过，仅保留既有 Vite 主 chunk 体积 warning。当前提交、推送、合并和部署待发布操作完成后补录 SHA 与健康检查。
...
- 后端聚焦合同/适配器/权威评分/worker/纠偏回归：`169 passed`。
- 后端全量（Python 3.12、全新临时 `DATA_DIR`）：`1658 passed, 1 skipped, 6 warnings`；warning 为既有 FastAPI/httpx 与 PDF SWIG 弃用提示。
- 前端节点纠偏、机制编辑器、基准纠偏与纠偏目的合同脚本通过；TypeScript 检查和 Vite 生产构建通过，仅保留既有主 chunk 大于 500 kB 提示。

## 最新实施：NAS 原图只读引用（2026-08-19，本地候选待发布）
...
- 存量基准回归、增量审核、候选回归统一使用各自运行快照中的纠偏合同；合同包含调用 A、调用 B、等级撮合器（V3）节点、中文说明、取值约束、证据要求、语义版本和服务端重算引用。
- 新增第 75 版增量迁移，为三类运行保存规范合同和哈希；历史运行不读取现役合同补齐，原有评测、模型响应、人工真值和审核轮次保持不变。
- 纠偏页面改为按合同动态渲染，支持新增/删除/类型变化/语义变化的明确状态、跨轮次兼容继承、再次编辑和 V3 服务端重算；历史归档字段不完整时明确只读并列出不可用节点。
- 增量审核和候选回归已接入同一纠偏视图与提交边界；候选启用只返回机制刷新信息，不自动启用新机制、不影响已打开旧回归的冻结合同。
- 本地验证：后端全量 `1605 passed, 1 skipped, 6 warnings`；前端生产构建、合同渲染、候选刷新、基准纠偏工作台和原图浮层检查全部通过。警告仅为既有依赖弃用、Vite 配置和主资源体积提示。
- 当前仍未推送、未合并、未部署测试服，未调用真实模型、未执行 2200 张重跑、未迁移 NAS；真实浏览器和测试服验收留待发布窗口。

## 最新实施：灵感图纠偏等级语义与连续工作台（2026-08-18，本次发布候选）

- 修复 V3 人工维度纠偏的等级反转：冻结阈值按高分为好、L1 最优、L5 最差重算；旧引擎结果保持兼容。
- 新增审核轮次重开与历史纠偏回显，旧轮次只追加不覆盖；重开时隐藏当前盲投但保留上一轮作为编辑初值。
- 基准回归纠偏补齐左图右编辑、自动进入下一条、手动“下一个”、再次修改和 V3 节点纠偏入口。
- 第 74 版迁移兼容历史最小 SQLite 库；3D/SU 并发工作树未改动。
- 本地验证：后端专项 91 passed；前端纠偏/节点合同、TypeScript 检查和 Vite 生产构建通过。当前等待本次 Codeup 合并与测试服部署验收。
...
- 最终方案已按“背景与现状 → 当前痛点 → 解决方案”展开，统一口径为：特鹏标签中台（Label System）就是新标签体系的事实产品和标签/内容中台通用底座，不再建设第二套标签后台。
...
## 最新实施：合同自有评分原语与自定义配置兼容（2026-08-20，本地待发布）

- 修复运营创建维度候选时的三类合同阻塞：媒介降权不再限定四个旧 key；关闭的高分 veto
  不再强制携带 `A/B/record_only` tiers；`requires_any_hard_defect`、`hard_defects` 和
  `image_defects` 改为合同拥有的业务字符串数组。
- 聚合器支持合同媒介 alias/fallback、可选修正节点和硬伤逐条扣分；旧 v1/v2 封顶合同仍按
  原有语义回放。运营编辑器改为读取合同中的媒介键，可新增、删除、改名并维护引用。
- 新候选 API、合同/红线/适配器/聚合器回归覆盖截图中的透明棋盘格、未完成手绘草稿、低价值
  平面图和自定义媒介场景；未修改历史 revision 或评测结果。
- 当前验证：后端全量 `1670 passed, 1 skipped, 6 warnings`；前端 TypeScript、机制编辑器/
  V3/维度/等级合同脚本和 Vite 生产构建通过。warning 仅为既有依赖弃用与主 chunk 体积提示。
- 架构决策见 ADR-0051。提交、推送和测试服部署待本次发布门禁完成后补录。
...
- 基准回归运行配置改为约 820px 的宽版纵向抽屉，分为“提示词配置 / V3 合同配置 / 执行方式”三段；启动动作和 A/B/V3 冻结摘要移入固定底栏，移除依赖整页 viewport 的六列布局，桌面宽度下不再出现单字竖排和合同摘要挤压。
- V3 合同配置支持当前现役与同类目候选版本选择。候选只接受仍连接当前现役祖先链的 `candidate`；`draft`、`retired` 和脱链候选只读禁选。选候选后自动切换 A/B 手动模式并按合同绑定版本预选；人工改动不匹配时启动按钮明确禁用，后端仍执行最终校验。
- active 回归请求不携带 `candidate_revision_id`，候选请求携带所选 revision；历史回归序列化保留冻结的 revision、candidate revision ID 和 contract hash，不读取后续现役状态覆盖历史证据。
- 主结果区恢复完整 L1–L5 五档 5×5 分布矩阵，行末展示单档召回率，底部展示单档准确率（Precision）；零分母显示 `—`。原有精确等级准确率、相邻等级准确率和推荐档（L1/L2）、常规档（L3/L4）、过滤档（L5）三档聚合全部保留。
- 本批不新增数据库迁移，不改变激活/发布/存量重跑/真实模型调用/自动组批门禁；不推送、不合并、不部署测试服，等待后续精确授权。

当前验证：

- 后端全量：`1338 passed, 1 skipped, 6 warnings`（Python 3.12；命令使用 `PYTHONPATH=.`，警告为既有依赖弃用）。
- 前端 `npm run lint`、`npm run build`、V3 运行配置合同、V3 UI 合同和五档矩阵合同全部通过；构建仅保留既有主 chunk 大于 500 kB 的 Vite warning。
- 关键提交：`42ab77b`（实施计划）、`6c49bf5`（V3 revision 类型/API/纯函数）、`7451a12`（宽版抽屉与 A/B 联动）、`4534d58`（五档矩阵）、`835b07d`（历史冻结 V3 元数据）、`a16e9b0`（单提示词模式保持 B 为空）。

## 最新实施：回归等级聚合指标、原图检查背景与机制投影修复（2026-08-14）

- 基准回归保留精确等级准确率、相邻等级准确率和完整 L1–L5 混淆矩阵，并额外展示推荐档
  `L1/L2`、常规档 `L3/L4`、过滤档 `L5` 的精确率与召回率。聚合指标由当前选中历史轮次的
  冻结五档矩阵动态计算，不新增后端指标口径，也不替代原矩阵。
- 原图 Lightbox 使用明暗棋盘格检查背景和清晰图片边缘轮廓，保持 `object-contain`、无滤镜、
  不裁切且继续使用原始素材 URL，便于人工识别白边、透明边缘和画布边界。
- `model_3d_su` 启动 seed 会为运行时合同创建或修复不可变投影；同 revision 的历史冻结产物若与
  运行时事实不一致，会退役旧 active revision、追加明确的新 revision 并更新指针，不再仅凭版本号
  复用陈旧内容。非空指针指向的冻结产物不一致时 fail-closed；operator-owned 合同不会被系统 seed
  自动挂接或覆盖。
- 候选启用和系统合同升级会同步运行时 provenance，使 `created_by` 与当前冻结 revision 一致；
  人工审批身份仍由纠偏/发布记录单独追溯，不混入候选产物作者字段。

当前验证：

- 后端全量：`1337 passed, 1 skipped, 6 warnings`；专项覆盖同 revision 陈旧产物、非空错误指针、
  operator-owned 空指针、候选启用 provenance，以及灵感图/PDF/3D-SU seed 幂等升级。
- 前端全部既有合同、Lightbox 浏览器契约、等级指标浏览器契约、TypeScript lint 和 Vite production
  build 通过；等级指标契约覆盖历史轮次切换和实际展开的 L1–L5 矩阵。
- Lightbox 的滤镜、边框与历史轮次刷新均执行过变异检查；构建只保留既有主 chunk 大于 500 kB
  的 Vite warning。
- 发布继续遵循 combined main 唯一部署原则：不得直接部署本地旧提交，不启动真实评测、纠偏、
  存量重跑、自动组批或模型调用。

## Combined release：3D/SU 与灵感图纠偏能力同包合流（2026-08-14）

- 本次合流同时保留并交付两条能力：3D/SU 模型美感评测机制，以及灵感图人工节点纠偏、理由和证据驱动的调用 A、调用 B、V3 路由能力。
- 合流基线为 Codeup `main@f7539beb09d5d780f830e3f0f96590c49fbc610a`，叠加纠偏提交 `c39cf31a730ad5189eff0ca7d8389614b52d60e8`；两侧原有状态记录均保留，未删除或改写。
- 本次仅完成代码合流、验证和 Codeup 合并请求；测试服 `192.168.1.35:8081` 尚未部署，后续由统一部署任务在最终 `main` SHA 确认后执行。
...
## 最新实施：灵感图调用 B v2 与语义软封顶候选（2026-08-14）

- 已用运行 20/23 的冻结 A/B 输出做交叉重放：修订 11 是 L5 召回提升的主要来源；现有补强调用 B 与第五锚没有产生可证明的五档净收益。运行 23 的八维方向缺少合同声明，且提示词输出示例为八维全 1，形成高质量图也输出低等级维度的明确诱导。
- 新调用 B v2 草稿明确 `grade=5` 最好、`grade=1` 最差，并把输出示例改为中性 `grade=3`；调用 B 退出红线、内容范围与最终等级职责，只保留美感基础分、逐维证据和不足项。
- 新候选质量规则兼容历史 `cap_to=59` 精确回放，同时支持 `cap_to_level=L4` 的语义上限；在 90/80/76/60/0 阈值下动态解析为 75。只有随手拍同时满足呈现完整性与灵感参考价值均不高于 2 时，才窄门升级到 L5 的 59 分上限。
- 现役 revision 9、运行 20/23、人工真值、模型、调用 A 和 2,200 张全量回归均未修改。新提示词仍是离线草稿，新 V3 规则尚未部署或创建远端候选。

当前验证：

- 语义软封顶与调用 B v2 的测试均经历红灯再绿灯；聚焦回归 `102 passed, 1 warning`。
- 完整后端套件输出 `1290 passed, 1 skipped, 6 warnings`；首次壳层回执因 zsh 只读变量名在 pytest 完成后报错，部署前必须用修正后的命令重新取得退出码 0。
- 根目录候选/五档工具测试 `24 passed`；代码与提示词不含凭据。

下一步：提交本次候选代码，使用受保护脚本部署测试服，创建新的草稿提示词和 candidate revision，仅运行固定 100 张门禁。门禁未通过时停止，不启动 2,200 张。

## 最新实施：灵感图第五锚图与未发布候选小样本门禁（2026-08-13）

- 当前隔离分支为 `codex/l5-anchor-candidate-gate-20260814`，基于测试服当前提交 `7575e6415ce89ece4a66a155672146a44de6b8ff`。本批只为灵感图调用 B 候选增加第五张 L5 Owner 锚图，并允许普通基准回归显式冻结一个未发布 V3 候选修订；不修改 R0～R4、人工真值、现役提示词、现役 V3 投影、模型、映射或存量标签事实。
- 现役四锚合同继续使用内部存储名且不查询资产表。公开五锚候选只冻结 `asset_id`、等级、MIME 与 SHA-256，Worker 从资产表解析服务端存储名，并在调用模型前对合同、资产表与原图摘要关闭失败；跨平台路径分隔符、绝对路径、`.` 与 `..` 均拒绝。
- `POST /api/baseline-sets/{baseline_set_id}/runs` 新增可选 `candidate_revision_id`。默认不传时仍冻结现役合同；传入时只接受同类目、状态为候选、且祖先链仍指向当前现役投影的修订。候选完整合同与基线现役修订证据写入运行和作业快照，不改变现役投影；跨类目、非候选、投影漂移或祖先链损坏均关闭失败。
- 候选合同与执行提示词新增三层绑定门禁：人工创建候选回归时，合同 `prompt_bindings.call_a_version`、`prompt_bindings.call_b_version` 及灵感图 `aesthetic_foundation.call_b_version` 必须与实际 A/B 提示词版本一致；自动纠偏先创建草稿提示词，再按实际 A/B 组合冻结不可变候选修订；Worker 在任何模型调用前再次交叉核验候选合同与 StrategyBundle，不一致时关闭失败。
- 本批候选仍是未发布实验资产：创建候选修订与草稿 B 提示词后只允许先跑小样本门禁。门禁顺序固定为过滤层、推荐层、五档与疑难案例；通过前不得启动 2,200 张全量回归，也不得启用候选修订或发布草稿提示词。

当前验证：

- 候选回归边界按测试先行完成，四项定向行为均经历红灯再绿灯：候选冻结且现役投影不变、跨类目拒绝、非候选拒绝、现役投影漂移拒绝。
- 第五锚图、V3 权威 Worker、候选 revision、提示词绑定与基准回归聚焦集合：`103 passed, 1 warning`。其中 Worker 漂移回归确认提供方客户端构造次数为 0。
- 后端全量：`1286 passed, 1 skipped, 6 warnings`（Python 3.12）。
- 前端 TypeScript 与 Vite 正式构建通过；仅保留既有主包大于 500 kB 的警告。
- 独立复审已确认上轮提示词绑定 Important 完整关闭，未发现新的 Critical 或 Important，结论为可部署。
- 尚未部署测试服，尚未创建真实候选修订、草稿提示词或小样本回归；这些动作只在实时活动任务、数据库完整性、服务器提交和回退快照再次核验后执行。
...
# 2026-08-19｜纠偏候选差异合成与自动修复（待合并）

- 基于 `origin/main@8f9d0370cf850e01dd8be80cdefe3e90e40ecac9` 在隔离工作树实现，未触碰主工作区既有修改。
- 调优模型现在只需返回需要变化的提示词和 revision JSON Merge Patch；未返回的提示词正文、等级合同、分类映射和类目维度从冻结现役版本继承。显式删除或非法类型不会被静默补回，完整合成后仍执行机制 profile、分类映射和维度合同严格校验。
