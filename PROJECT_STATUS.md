...
- 当前隔离分支为 `codex/l5-anchor-candidate-gate-20260814`，基于测试服当前提交
  `7575e6415ce89ece4a66a155672146a44de6b8ff`。本批只为灵感图调用 B 候选增加
  第五张 L5 Owner 锚图，并允许普通基准回归显式冻结一个未发布 V3 候选修订；不修改
  R0～R4、人工真值、现役提示词、现役 V3 投影、模型、映射或存量标签事实。
- 现役四锚合同继续使用内部存储名且不查询资产表。公开五锚候选只冻结
  `asset_id`、等级、MIME 与 SHA-256，Worker 从资产表解析服务端存储名，并在调用模型前
  对合同、资产表与原图摘要关闭失败；跨平台路径分隔符、绝对路径、`.` 与 `..` 均拒绝。
- `POST /api/baseline-sets/{baseline_set_id}/runs` 新增可选
  `candidate_revision_id`。默认不传时仍冻结现役合同；传入时只接受同类目、状态为候选、
  且祖先链仍指向当前现役投影的修订。候选完整合同与基线现役修订证据写入运行和作业快照，
  不改变现役投影；跨类目、非候选、投影漂移或祖先链损坏均关闭失败。
- 候选合同与执行提示词新增三层绑定门禁：人工创建候选回归时，合同
  `prompt_bindings.call_a_version`、`prompt_bindings.call_b_version` 及灵感图
  `aesthetic_foundation.call_b_version` 必须与实际 A/B 提示词版本一致；自动纠偏先创建
  草稿提示词，再按实际 A/B 组合冻结不可变候选修订；Worker 在任何模型调用前再次交叉核验
  候选合同与 StrategyBundle，不一致时关闭失败。
- 本批候选仍是未发布实验资产：创建候选修订与草稿 B 提示词后只允许先跑小样本门禁。
  门禁顺序固定为过滤层、推荐层、五档与疑难案例；通过前不得启动 2,200 张全量回归，
  也不得启用候选修订或发布草稿提示词。

当前验证：

- 候选回归边界按测试先行完成，四项定向行为均经历红灯再绿灯：候选冻结且现役投影不变、
  跨类目拒绝、非候选拒绝、现役投影漂移拒绝。
- 第五锚图、V3 权威 Worker、候选 revision、提示词绑定与基准回归聚焦集合：
  `103 passed, 1 warning`。其中 Worker 漂移回归确认提供方客户端构造次数为 0。
- 后端全量：`1286 passed, 1 skipped, 6 warnings`（Python 3.12）。
- 前端 TypeScript 与 Vite 正式构建通过；仅保留既有主包大于 500 kB 的警告。
- 独立复审已确认上轮提示词绑定 Important 完整关闭，未发现新的 Critical 或 Important，
  结论为可部署。
- 尚未部署测试服，尚未创建真实候选修订、草稿提示词或小样本回归；这些动作只在实时活动
  任务、数据库完整性、服务器提交和回退快照再次核验后执行。
