...
## 当前进行中：维度管理器 P2-S1 路由基础（2026-07-31）

> 隔离分支：`dim-p2-s1-route-foundation`；基线：
> `main @ 83f5e47cf94f1ae2fb359e340b38e09ccd476aff`。
> 本阶段只交付候选路由合同、确定性解析器和只读管理界面，不接生产 Worker，
> 不修改正式数据、生产指针或部署实例。
...
- 单品候选包设置最少 50、目标 100 张校准门，并要求目标错例、稳定对照、
  ...
  fail-closed。
- 新增只读路由策略 API 和“优化与回归 → 维度管理器”页面，展示候选版本、
...
> 验收收口提交基线为 `513ed67`。本链已经 Owner 授权合入
> `main @ 83f5e47`，同步 Windows 分支并部署到生产
> `windows-deploy @ 32872cd`。
...
- P1-S1～S6 的代码、迁移、冻结回放、后端全量测试、前端合同、lint、build
  和真实浏览器门禁均已通过。
- P1 已合入主干、同步 Windows 分支并在 MacBook 生产通过蓝绿部署验收；
  生产数据库迁移至 27，468 张图片数量、大小和 SHA-256 均保持一致。
