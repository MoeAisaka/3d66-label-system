...
- 当前本地 Git 仍保留既有 GitHub 远程，尚未推送本轮未提交改动；切换默认
  push remote 前先验证公司网络与 Codeup 鉴权。
- 分支关系：每次交付以 `git status -sb` 的实时结果为准。

## 最新完成：可解释基准回归与连续优化工作流（2026-07-31）

> 本轮开发基线：`7331399`。代码与隔离浏览器验收已完成，待提交、推送至
> Codeup 并部署到 MacBook 验证环境；公司 Windows 实例由公司前端工程师
> 后续按 `windows-deploy` 分支自行部署。
...
- 使用隔离 `DATA_DIR`、隔离端口、临时测试账号和两张测试素材完成真实 Chrome
  验收：1440px 桌面与 390px 移动端均无横向溢出、页面错误或非预期控制台
  错误；文件名 L1/L3 自动预填、L3 手改为 L2、创建冻结基准集、提示词取值
  入口、案例池下一步和组批配置说明均通过。测试未读取生产数据库、未调用模型。

仍待完成：

- 完成 Codeup 远程鉴权，核对远端历史与分支保护后推送 `main` 和
  `windows-deploy`；禁止对未知远端历史 force push。
- 在公司 MacBook 按既有蓝绿门禁部署并复验生产数据完整性、关键页面和控制台。
