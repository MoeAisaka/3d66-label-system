...
> 本轮开发基线：`7331399`。已推送至 Codeup：主干文档提交 `4f39eda`
> （功能与修复：`1a38114`、`0199890`、`df81fc1`）；本分支随主干同步，
> 精确提交号执行 `git log -1` 查看。两侧均为快进，未使用 force push。
> 已在 MacBook 以隔离 `DATA_DIR` 的 8099 金丝雀完成真实浏览器验收；生产 8080
> 仍为 `5c25bfa`，是否提升待操作员决定。公司 Windows 实例由公司前端
> 工程师后续按 `windows-deploy` 分支自行部署。
...
补充修复：基准回归页横向溢出（提交 `0199890`、`df81fc1`）
...
- MacBook 金丝雀真实浏览器实测文档级横向溢出（修复后 / 修复前生产
  ...
  1280px 残留 107px 属既有缺陷，来自素材表 `min-w-[720px]` 与“创建基准集”
  按钮（两者在修复前生产上 `right` 值完全相同），本轮未触及、不在本次范围。
...
- 决定是否将金丝雀 `3d66-label-system.green-daa0188-20260731T101802Z`
  ...
  实例，属中高风险，需操作员显式确认。
- 修复 1280px 残留的既有横向溢出（素材表 `min-w-[720px]`、创建按钮）。
