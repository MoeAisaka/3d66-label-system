...
try {
    ...
    Assert-NoReparsePoint -Path $PSCommandPath -Label '备份脚本路径'
    ...
    $arguments = @('-X', 'utf8', '-m', 'app.windows_deploy', 'backup', '--repo-root', $repoRoot)
    ...
    [Console]::Error.WriteLine("备份失败：$($_.Exception.Message)")
    exit 1
}
