...
try {
    ...
    Assert-NoReparsePoint -Path $PSCommandPath -Label '恢复脚本路径'
    ...
    $arguments = @(
        '-X', 'utf8', '-m', 'app.windows_deploy', 'restore',
        '--repo-root', $repoRoot, '--backup', $Backup
    )
    ...
    if ($validationExitCode -ne 0) {
        [Console]::Error.WriteLine("恢复已阻止：只读校验未通过（退出码 $validationExitCode）。")
    ...
    [Console]::Error.WriteLine("恢复失败：$($_.Exception.Message)")
    exit 1
}
