...
7. 来源身份按站点绑定：国内首切片使用 `dim_res_info_union.ll_id` 与 `res_type in (1,6)`，海外使用 `ods_ll_relebook_res.res_id` 与 `res_type=6`，海外 `is_single` 来自 `ods_ll_relebook_res_su_extra`。两套来源分别探查、分别生成证据哈希；海外主表与 extra 表的关联键未签认前，不允许真实接入。本 ADR 只记录架构边界，不扩大当前冻结执行合同。
...
- 国内与海外不得复用一套未经验证的素材主键：国内候选键为 `(res_type,ll_id)`，海外候选键为 `(res_type,res_id)`；`is_single` 的来源表和关联关系必须进入来源证据。
