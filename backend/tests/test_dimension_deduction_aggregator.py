...
def test_rule_hits_accumulate_into_weighted_dimension_deductions() -> None:
    ...
    first = output["dimensions"][0]
    rules = config["common_group"]["schema_definition"]["dimensions"][0]["deduction_rules"]
    first["hit_rules"] = [
    ...
    assert composed["evidence"][first["dimension_key"]]["applied_rule_deduction"] == sum(
