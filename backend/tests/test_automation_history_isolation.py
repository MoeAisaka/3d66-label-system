...
def test_review_round_migration_follows_global_automation_contract():
    ...
    assert MIGRATIONS[-3].version == 73
    assert MIGRATIONS[-3].name == "add_global_automation_lanes"
    assert MIGRATIONS[-2].version == 74
    assert MIGRATIONS[-2].name == "add_review_rounds"
    assert MIGRATIONS[-1].version == 75
    assert MIGRATIONS[-1].name == "add_correction_contract_snapshots"
